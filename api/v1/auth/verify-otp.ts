...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  try {
    ...
    const { email, code, verificationToken } = body || {};
    if (!email || !code) {
    ...
    // 1. Try stateless cryptographic verification token first (survives cross-instance serverless calls)
    ...
    console.error('Unhandled error in /api/v1/auth/verify-otp:', err?.message || err);
    return res.status(500).json({ success: false, message: 'Verification error occurred. Please try again.' });
  }
}
